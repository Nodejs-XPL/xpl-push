...
const GCM_SERVER_URL = "https://gcm-http.googleapis.com/gcm/send";
...
class GCMChannel extends PushChannel {
  ...
  constructor(store, rule) {
    ...
    this._groupKeySemaphore=Semaphore();
  }
  ...
  _pushMessages(messages, callback) {
    ...
    this._store.listClients('gcm', rule.id, (error, list) => {
      ...
      var msg={ data: {msg: messages} };

      if (true || list.length>1) {
        var rids=list.map((client) => client.pushURL);
        msg.registration_ids=rids;
        
      } else {
        msg.to = list[0].pushURL;
      }
      ...
      var params={ url: GCM_SERVER_URL, 
          ...
          headers: {
            ...
            "Authorization": "key="+rule.gcm.apiKey
          },
      ...
      request(params, (error, response, body) => {
        ...
        debug("_pushMessages", "statusCode=",response.statusCode,"statusMessage=",response.statusMessage,"body=",body);
